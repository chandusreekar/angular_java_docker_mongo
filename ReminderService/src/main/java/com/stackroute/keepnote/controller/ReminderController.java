...
public class ReminderController {
	...
	public ResponseEntity<Reminder> createReminder(@RequestBody Reminder reminder) {
		...
		try {
			...
			return new ResponseEntity<Reminder>(reminderData, HttpStatus.CREATED);
		} catch (Exception e) {
		...
		try {
			...
			try {
				...
				if (null != UpdateReminder)
					return new ResponseEntity<Reminder>(UpdateReminder,HttpStatus.OK);
				else
					throw new Exception();
			} catch (Exception e) {
				return new ResponseEntity<Reminder>(HttpStatus.NOT_FOUND);
	...
	public ResponseEntity <List<Reminder>> getReminderByUserId(@PathVariable("userId") String userId)
			throws ReminderNotFoundException {

		try {
			List<Reminder> reminderData = reminderService.getReminderByUserId(userId);
			if (null != reminderData) {

				return new ResponseEntity<List<Reminder>>(reminderData, HttpStatus.OK);
			} else {
				System.out.println("inside reminder not found");
				return new ResponseEntity<List<Reminder>>(HttpStatus.NOT_FOUND);
			}
		} catch (Exception e) {
			System.out.println("find reminder exception occured");
			return new ResponseEntity<List<Reminder>>(HttpStatus.NOT_FOUND);
	...
	@GetMapping("/hello")
	public String greet() {
		return "hello from reminderservice";
	}
}
