...
public class NoteController {
	...
	public ResponseEntity<Note> createNote(@RequestBody Note note) {
		...
		try {
			 List<Reminder> rem = note.getReminders();
			 if (rem != null) {
				 System.out.println("remender data is "+rem.get(0));
			 }else {
				 System.out.println("in note remender data is null");
			 }
			System.out.println("inside note controller and printinhg note data "+note);
	...
	public ResponseEntity<List<Note>> getAllNoteByUserId(@PathVariable("id") String userId)
		...
		try {
			...
			System.out.println("inside notecontroller size of notedata is"+noteData.size());
			return new ResponseEntity<List<Note>>(noteData,HttpStatus.OK);
	...
	@GetMapping("/hello")
	public String greet() {
		return "welcome from note service";
	}
